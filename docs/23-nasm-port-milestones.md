...
| N4A.3 | size solution from N0 numbers: (a) accept mini-assembler as the on-image tool, or (b) grow `VOL_SECTORS`/`IMG_MB` via the layout block + `check-layout` + `bochsrc` CHS, or (c) ship `NASM.COM` on `dos64-tools.img`. Never squeeze the 176-sector kernel slot | S | decision recorded + image boots |
