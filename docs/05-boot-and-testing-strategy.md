...
0x00007E00  Stage2 (≈8 KiB, performs mode switch, loads kernel)
0x00008E00  Stack (temporary, grows down)
0x00010000  Page tables: PML4 0x1000, PDPT 0x2000, PD 0x3000, PT 0x4000 (aligned)
             Identity map 0–4MiB (or 0–2MiB + kernel at 1MiB); use 2MiB pages on PD if wanted
0x00100000  Kernel entry (flat binary, 64-bit)
...
4. Load Stage2: use BIOS `INT 13h AH=42h` LBA extended read if available, else CHS (`AH=02h`). Stage2 at 0x7E00, ~16 sectors (8 KiB). Verify signature 0xAA55.
5. `jmp 0:0x7E00`.
...
**Page-table details:** use `dq` entries with flags `P|RW` (0x3). For paging bootstrap, identity map first 2 MiB → needed for stage2. Kernel text at 1 MiB falls inside. For simplicity identity map first 4 GiB via 2 GiB PD entries (`PSE` 2 MiB) to avoid PT.
...
lidt [idt_ptr]
```
Needed handlers: #DE(0), #GP(13), #PF(14) → fault print to VGA then hlt; IDT gate 0x21 for DOS syscall, IRQ1 keyboard.
...
2. **Keyboard** – port 0x60 data, 0x64 status (OBF 1, IBF 2); translate scancode set 1 to ASCII; circular QUEUE (80B) via `rearp/front`. Verify echo.
...
*Stage 5 – FS.* Format `dos64.img` as FAT12 (10 MiB, `mkfs.fat -F 12` or use `mformat`), place original directory entries, call `FATREAD`/`GETENTRY` from 64-bit kernel.

*Stage 6 – Syscalls.* Exercise `INT 21h` gate for `AH=09` print string.

*Stage 7 – Shell.* Replace COMMAND transient with minimal prompt loop reading via `BUFIN` (still `INT 21h` 0Ah).
...
nasm -f elf64 src/kernel/*.asm -o build/kernel.o
ld -T linker.ld -o build/kernel.bin  # linker places .text at 0x100000
