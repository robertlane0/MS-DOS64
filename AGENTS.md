...
> N4B-pre + N4B (PLAN.md item 10) done 2026-09-12: kernel slot `224→256`
> sectors (extent `[16,272)`, test 82 locks it); native `ASM64.COM`
> mini-assembler (`src/tools/`: pure `asm64_core` two-pass `-f bin`
> subset + `asm64_main` CLI over N2 handles, test 93 PURE hello
> round-trip in-harness, `make asm64-check` 4/4 byte-identical host-side;
...
- **Kernel load**: `KERNEL_SECTORS 256` (LBA 16+, extent [16,272), clear of scratch `400`/`500–511` and the FAT12 volume at LBA 512+ stamped by `tools/mkfat12.py`)
...
- [x] Jump to 64-bit kernel entry point (`0x100000`, staged via `0x70000`, `KERNEL_SECTORS 256`)
...
> final `0x100000` (`KERNEL_SECTORS 256`); initial `RSP` `0x90000`;
...
| `0x100000+` | Kernel (linked flat at 1 MiB, ~115 KiB / ~230 sectors, ≤256) |
...
- Canonical values live only in the Makefile disk-layout block
  ...
  `KERNEL_LBA=16`, `KERNEL_SECTORS=256`), which generates
...
| 1–15 | Stage2 |
| 16+ | Kernel binary (up to 256 sectors) |
